        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {services.map((service) => (
            <motion.div
              ...
              viewport={{ once: true }}
              transition={{ duration: 0.5 }}
              className={`bg-white p-8 cursor-pointer transition-all duration-500 ease-in-out rounded-lg ${
                expandedId === service.id ? 'min-h-[200px]' : 'min-h-[100px]'
              }`}
              onClick={() => setExpandedId(expandedId === service.id ? null : service.id)}
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="sm:text-5xl text-4xl text-gray-600 font-semibold">{service.title}</h3>
                <motion.span
